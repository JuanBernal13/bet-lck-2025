...
class Visualizer:
    ...
    def plot_eda(team_rows: pd.DataFrame):
        ...
        fig.suptitle('Análisis Exploratorio LCK 2025', fontsize=18, fontweight='bold', y=1.05)
        ...
        axes[0].set_title('Win Rate por Equipo', fontsize=14)
        ...
        axes[1].set_title('Win Rate por Lado', fontsize=14)
        ...
        axes[2].set_title('Inercia de Meta por Split', fontsize=14)
    ...
    def plot_correlations(corr: pd.Series):
        ...
        plt.title('Correlación de Métricas In-Game con la Victoria (LCK 2025)', fontsize=16, fontweight='bold', pad=20)
        plt.tight_layout()
        plt.show()
    ...
    def plot_calibration(y_test, raw_proba, cal_proba, model_name: str):
        ...
        fig.suptitle(f'Calibración de Probabilidades — {model_name}', fontsize=16, fontweight='bold')

        for proba, label, style, color in [
            (raw_proba, 'Modelo Base',             's--', GRAY),
            (cal_proba, 'Calibrado (Platt Scaling)', 'o-', AMBER),
        ]:
        ...
        axes[0].plot([0, 1], [0, 1], 'k--', alpha=0.3, label='Calibración Perfecta')
        axes[0].set_xlabel('Probabilidad Predicha (Equipo Blue)', fontsize=12)
        axes[0].set_ylabel('Fracción de Victorias Reales', fontsize=12)
        axes[0].legend(fontsize=11)
        axes[0].grid(True, alpha=0.2)

        axes[1].hist(raw_proba, bins=25, alpha=0.4, label='Base', color=GRAY,  density=True, edgecolor='white')
        axes[1].hist(cal_proba, bins=25, alpha=0.6, label='Calibrado', color=BLUE, density=True, edgecolor='white', hatch='//')
    ...
    def plot_bankroll(history: list, ev_dist: pd.Series, ev_min: float):
        ...
        fig.suptitle('Análisis de Gestión de Riesgo — Quarter-Kelly', fontsize=16, fontweight='bold')

        fill_color = GREEN if history[-1] >= history[0] else RED
        axes[0].plot(history, linewidth=3, color=BLUE, label='Bankroll Algorítmico')
        axes[0].axhline(history[0], ls='--', color=GRAY, alpha=0.5, label='Fondo Inicial')
        axes[0].fill_between(range(len(history)), history[0], history, alpha=0.1, color=fill_color)
        axes[0].set_xlabel('Apuestas Ejecutadas', fontsize=12)
        ...
        axes[1].axvline(ev_min, color=AMBER, ls='--', linewidth=2.5, label=f'Umbral EV = {ev_min:.0%}')
